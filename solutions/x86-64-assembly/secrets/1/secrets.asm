...
extract_higher_bits:
    ...
    mov ax, di      ; Load the 16-bit value into ax
    shr ax, 8       ; Shift right 8 bits - high byte moves to low byte
    movzx eax, al   ; Zero-extend to 32-bit for return
    ret
...
extract_lower_bits:
    ...
    mov ax, di      ; Load the 16-bit value into ax
    shl ax, 8       ; Shift left 8 bits - low byte moves to high byte
    movzx eax, ah   ; Zero-extend to 32-bit for return
    ret
...
format_private_key: 
    ...
    xor edx, eax                 ; find bits in the base value that are also set in the mask
    not edx                      ; flip all bits in the result
    mov eax, edx
    ret
